...
export abstract class ScuffrSvgScript<TScript extends BlockScript = BlockScript> extends ScuffrSvgElementParent implements ScuffrReferenceLink<ScuffrSvgBlock>, ScuffrPointAttachable {
    ...
    protected _init(blocks?: ScuffrSvgBlock[]) {
        if (blocks) {
            ...
            for (let i = 0; i < this.script.blocks.length; i++)
                this.children.push(new ScuffrSvgBlockInstance(this.script.blocks[i], { index: i, parent: this }));
        }
    }
    ...
    public spliceBlocks(start: number, deleteCount?: number, items?: ScuffrSvgBlock[], update: boolean = true): ScuffrSvgBlock[] {
        ...
        if (!items || items.length === 0) {
            ...
            this.script.spliceBlocks(start, deleteCount);
        } else {
            splicedChildren = this.children.splice(start, deleteCount ?? 0, ...items);
            this.script.spliceBlocks(start, deleteCount ?? 0, items.map(ele => (<ScuffrSvgBlockInstance>ele).block));
