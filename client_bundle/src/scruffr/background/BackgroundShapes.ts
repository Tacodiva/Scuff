...
export const BackgroundShapes = {
    InputRound: new class extends ScruffrBackgroundShape {
    ...
    InputTriangle: new class extends ScruffrBackgroundShape {
        public override createPath(size: Vec2, lines: ScruffrBackgroundContentLine[]): string {
            if (lines.length !== 1 || lines[0].modifier)
                throw new Error("Round shaped blocks do not support multiple lines.");
            let radius = size.y / 2;
    ...
    StackBody: new ScruffrStackableBackgroundShape(),
    ...
    StackHead: new class extends ScruffrStackableBackgroundShape {
        ...
        public override updateElement(element: SVGElement, size: Vec2, lines: ScruffrBackgroundContentLine[], verticalPadding: number, background: ScruffrBackground): void {
